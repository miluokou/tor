	...
	<view class="container">
		...
		<view class="tui-header-screen">
			...
			<view class="tui-screen__box">
				<view class="tui-screen-item" :class="[tabIndex == -1 ? 'tui-active tui-bold' : '']" @tap="screen"
					data-index="-1">综合</view>
				<view class="tui-screen-item" :class="[tabIndex == 0 ? 'tui-active tui-bold' : '']" data-index="0"
					@tap="screen">
					<view>价格</view>
				...
				<view class="tui-dropdownlist" :class="[selectH > 0 ? 'tui-dropdownlist-show' : '']"
					...
					<view class="tui-dropdownlist-item" :class="[item.selected ? 'tui-bold' : '']"
			...
			<view class="tui-product-list">
				<view class="tui-product-container">
					<template v-for="(item, index) in productList">
						<!--商品列表-->
						<view :key="index" v-if="(index + 1) % 2 != 0 || isList" class="tui-pro-item"
							:class="[isList ? 'tui-flex-list' : '']" hover-class="tui-hover" :hover-start-time="150"
							@tap="detail">
							<image :src="'/static/images/mall/product/' + item.img + '.jpg'" class="tui-pro-img"
								:class="[isList ? 'tui-proimg-list' : '']" mode="widthFix"></image>
				...
				<view class="tui-product-container" :class="{'tui-pro--hidden':isList}">
					<template v-for="(item, index) in productList">
						<!--商品列表-->
						<view :key="index" v-if="(index + 1) % 2 == 0" class="tui-pro-item"
							:class="[isList ? 'tui-flex-list' : '']" hover-class="tui-hover" :hover-start-time="150"
							@tap="detail">
							<image :src="'/static/images/mall/product/' + item.img + '.jpg'" class="tui-pro-img"
								:class="[isList ? 'tui-proimg-list' : '']" mode="widthFix"></image>
			...
			<view class="tui-drawer-box" :style="{ height: drawerH + 'px' }">
				...
				<view class="tui-attr-btnbox">
					<view class="tui-attr-safearea">
						...
						<view class="tui-drawer-btn">
							<tui-button type="danger" height="60rpx" shape="circle" :size="26" @click="closeDrawer">
								确定(80万+件商品)</tui-button>
						</view>
					</view>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			detail() {
				this.tui.href('../groupDetail/groupDetail')
			}
		}
	}
</script>
