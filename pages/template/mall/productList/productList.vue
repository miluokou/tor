	...
	<view class="container">
		...
		<view class="tui-header-screen" :style="{ top: height + 'px' }">
			...
			<view class="tui-screen-bottom">
					<view v-for="(item, index) in attrArr" :key="index" class="tui-bottom-item tui-icon-ml"
		...
		<view class="tui-product-list" :style="{ marginTop: px(dropScreenH + 18) }">
			<view class="tui-product-container">
				<block v-for="(item, index) in productList" :key="index">
					<!--商品列表-->
					<view  v-if="(index + 1) % 2 != 0 || isList" class="tui-pro-item"
			...
			<view class="tui-product-container" :class="{'tui-pro--hidden':isList}">
				<block v-for="(item, index) in productList" :key="index">
					<!--商品列表-->
					<view v-if="(index + 1) % 2 == 0" class="tui-pro-item"
						:class="[isList ? 'tui-flex-list' : '']" hover-class="tui-hover" :hover-start-time="150"
						@tap="detail">
						<image :src="'/static/images/mall/product/' + item.img + '.jpg'" class="tui-pro-img"
							:class="[isList ? 'tui-proimg-list' : '']" mode="widthFix"></image>
