<template>
	<view class="container">
		<view class="tui-bg__box">
			<image :src="`${webURL}bg_seckill.png`" class="tui-bg__img" mode="widthFix" :style="{ opacity: opacity }">
			</image>
		</view>
		<view class="tui-header__bg">
			...
			<scroll-view class="tui-time-slot" scroll-x>
				<view class="tui-time__list" :class="{ 'tui-flex__between': timeSlot.length < 6 }">
					<view class="tui-time__item"
						:class="[timeSlot.length < 6 ? 'tui-flex__1' : 'tui-width__min', index == currentTab ? 'tui-time__active' : '']"
			...
			<view class="tui-list__goods">
				<view class="tui-goods__left" :class="{ 'tui-full__width': status == 3 }">
					<block v-for="(item, index) in productList" :key="index">
						<t-goods-item v-if="index % 2 == 0 || status == 3" :item="item" :status="status"
							:isList="status == 3"></t-goods-item>
					</block>
				</view>
				<view class="tui-goods__right" v-if="status !== 3">
					<block v-for="(item, index) in productList" :key="index">
						<t-goods-item v-if="index % 2 !== 0" :item="item" :status="status" :isList="status == 3">
						</t-goods-item>
					</block>
				</view>
			</view>
